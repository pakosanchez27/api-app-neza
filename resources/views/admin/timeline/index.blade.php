...
@section('content')
    @php
        $timelineCollection = method_exists($timelines, 'getCollection') ? $timelines->getCollection() : collect($timelines ?? []);
        $timelinesActivos = $timelineCollection->filter(fn($timeline) => (string) ($timeline->estatus ?? 1) === '1')->count();
        $timelinesInactivos = $timelineCollection->filter(fn($timeline) => (string) ($timeline->estatus ?? 1) === '0')->count();
    @endphp
    ...
    <div class="admin-shell mb-5 overflow-hidden">
        <div class="bg-[linear-gradient(135deg,#2f1821,#61102a)] px-6 py-6 text-white">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div class="max-w-3xl">
                    <p class="text-xs font-semibold uppercase tracking-[0.22em] text-[#ffd175]">Panel historico</p>
                    <h2 class="mt-3 text-2xl font-semibold">Lista de Antes y Despues</h2>
                    <p class="mt-2 text-sm leading-7 text-white/78">Consulta comparativas historicas, revisa su estatus y entra rapido a editar o eliminar desde una sola vista.</p>
                </div>
        ...
        <div class="grid gap-4 px-6 py-5 md:grid-cols-3">
            <div class="admin-stat">
                ...
                <p class="mt-3 text-4xl font-bold leading-none text-[#201815]">{{ $timelineCollection->count() }}</p>
                <p class="mt-2 text-sm text-[#6d5a62]">Comparativas registradas en el sistema.</p>
            </div>
            <div class="admin-stat">
                <p class="text-xs font-semibold uppercase tracking-[0.12em] text-emerald-700">Activas</p>
                <p class="mt-3 text-4xl font-bold leading-none text-[#201815]">{{ $timelinesActivos }}</p>
                <p class="mt-2 text-sm text-[#6d5a62]">Registros visibles actualmente.</p>
            </div>
            <div class="admin-stat">
                <p class="text-xs font-semibold uppercase tracking-[0.12em] text-rose-700">Inactivas</p>
                <p class="mt-3 text-4xl font-bold leading-none text-[#201815]">{{ $timelinesInactivos }}</p>
    ...
    <div class="admin-shell admin-table p-5">
        <div class="mb-5">
            ...
            <h3 class="mt-2 text-lg font-semibold text-[#201815]">Comparativas registradas</h3>
            <p class="mt-1 text-sm text-[#7d6870]">La tabla permite buscar, ordenar y revisar registros historicos de forma mas clara.</p>
        </div>

        <div class="overflow-x-auto">
            <table id="tabla-timeline" class="display stripe hover w-full text-sm">
                <thead>
                    <tr>
                        <th>ID</th>
                ...
                <tbody>
                    @forelse ($timelineCollection as $timeline)
                        @php
                            $estatus = $timeline->estatus ?? 1;
                        @endphp
                        <tr>
                            <td>{{ $timeline->id }}</td>
                            <td>
                                <div class="min-w-[220px]">
                                    <p class="font-semibold text-[#201815]">{{ $timeline->lugar_turistico ?? 'Sin nombre' }}</p>
                                    <p class="mt-1 text-xs text-[#7d6870]">Registro historico</p>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="flex items-center gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.timeline.destroy', $timeline) }}"
                                        class="form-eliminar-timeline inline-flex"
                                        data-timeline-titulo="{{ $timeline->lugar_turistico ?? 'este registro' }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            ...
                                            title="Eliminar" aria-label="Eliminar registro">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.8" stroke="currentColor" class="h-4 w-4">
                                                <path stroke-linecap="round" stroke-linejoin="round"
...
@push('scripts')
    ...
    <script>
        $(function() {
            ...
            $('.form-eliminar-timeline').on('submit', function(event) {
                event.preventDefault();

                const form = this;
                const titulo = form.dataset.timelineTitulo || 'este registro';
