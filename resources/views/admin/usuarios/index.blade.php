...
@section('content')
    @php
        ...
        $usuariosInactivos = $usuarios->where('activo', false)->count();
        $usuariosConFoto = $usuarios->filter(fn ($usuario) => filled($usuario->foto_perfil))->count();
    @endphp
    ...
    <div class="admin-shell mb-5 overflow-hidden">
        <div class="bg-[linear-gradient(135deg,#2f1821,#61102a)] px-6 py-6 text-white">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div class="max-w-3xl">
                    <p class="text-xs font-semibold uppercase tracking-[0.22em] text-[#ffd175]">Registros generales</p>
                    <h2 class="mt-3 text-2xl font-semibold">Usuarios</h2>
                    <p class="mt-2 text-sm leading-7 text-white/78">Consulta y administra a todos los usuarios con rol cliente dentro del sistema.</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid gap-4 px-6 py-5 md:grid-cols-3">
            <div class="admin-stat">
                ...
                <p class="mt-2 text-sm text-[#6d5a62]">Usuarios con rol cliente registrados.</p>
            </div>
            <div class="admin-stat">
                ...
                <p class="mt-2 text-sm text-[#6d5a62]">Cuentas que pueden iniciar sesion.</p>
            </div>
            <div class="admin-stat">
                <p class="text-xs font-semibold uppercase tracking-[0.12em] text-sky-700">Con foto</p>
                <p class="mt-3 text-4xl font-bold leading-none text-[#201815]">{{ $usuariosConFoto }}</p>
                <p class="mt-2 text-sm text-[#6d5a62]">Perfiles con imagen cargada.</p>
            </div>
        </div>
    </div>
    ...
    <div class="admin-shell admin-table p-5">
        <div class="mb-5">
            ...
            <h3 class="mt-2 text-lg font-semibold text-[#201815]">Usuarios registrados</h3>
            <p class="mt-1 text-sm text-[#7d6870]">Busca, ordena y ejecuta acciones rapidas sobre los usuarios del sistema.</p>
        </div>

        <div class="overflow-x-auto">
            <table id="tabla-usuarios" class="display stripe hover w-full text-sm">
                <thead>
                    <tr>
                        <th>Foto perfil</th>
                        <th>Nombre</th>
                        ...
                        <tr>
                                ...
                                <div class="flex min-w-[88px] justify-center">
                                    @if ($fotoPerfil)
                                        <img src="{{ $fotoPerfil }}" alt="Foto de {{ $nombre }}"
                                            class="h-14 w-14 rounded-full border border-[#eadde2] object-cover">
                                    @else
                                        <div
                                            class="flex h-14 w-14 items-center justify-center rounded-full bg-[#f3e6db] text-sm font-semibold uppercase text-[#7a2144]">
                                            {{ $iniciales ?: 'SN' }}
                                        </div>
                                    @endif
                                </div>
                            ...
                            <td>
                                <div class="min-w-[220px]">
                                    ...
                                    <p class="mt-1 text-xs {{ $usuario->activo ? 'text-emerald-700' : 'text-rose-700' }}">
                            ...
                            <td>
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.usuarios.edit', $usuario) }}"
                                        ...
                                        title="Editar" aria-label="Editar usuario">
                                    ...
                                    <form method="POST" action="{{ route('admin.usuarios.toggle-status', $usuario) }}"
                                        class="form-toggle-usuario inline-flex"
                                        data-usuario-nombre="{{ $nombre }}"
                                        data-is-active="{{ $usuario->activo ? '1' : '0' }}"
                                        data-action-label="{{ $usuario->activo ? 'desactivar' : 'activar' }}"
                                        data-action-title="{{ $usuario->activo ? 'Desactivar usuario' : 'Activar usuario' }}"
                                        data-action-text="{{ $usuario->activo ? 'El usuario dejara de poder iniciar sesion.' : 'El usuario volvera a poder iniciar sesion.' }}">
                                        @csrf
                                        @method('PATCH')
                                        <input type="hidden" name="motivo_desactivacion" value="">
                                        <button type="submit"
                                            class="inline-flex h-9 w-9 items-center justify-center rounded-full {{ $usuario->activo ? 'bg-rose-100 text-rose-700 hover:bg-rose-200' : 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' }} transition"
                                    ...
                                    <form method="POST" action="{{ route('admin.usuarios.reset-password', $usuario) }}"
                                        class="form-reset-usuario inline-flex"
                                        data-usuario-nombre="{{ $nombre }}">
                                        @csrf
                                        <button type="submit"
                                            class="inline-flex h-9 w-9 items-center justify-center rounded-full bg-amber-100 text-amber-700 transition hover:bg-amber-200"
                                            title="Reset password" aria-label="Enviar reset password">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.8" stroke="currentColor" class="h-4 w-4">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    d="M15.75 9V5.25A3.75 3.75 0 0 0 12 1.5 3.75 3.75 0 0 0 8.25 5.25V9m-.75 0h9a1.5 1.5 0 0 1 1.5 1.5v8.25a1.5 1.5 0 0 1-1.5 1.5h-9A1.5 1.5 0 0 1 6 18.75V10.5A1.5 1.5 0 0 1 7.5 9Z" />
                                            </svg>
                                        </button>
                                    </form>
                                </div>
...
@push('scripts')
    ...
    <script>
        $(function() {
            ...
            $('.form-toggle-usuario').on('submit', function(event) {
                ...
                if (isActive) {
                    Swal.fire({
                        ...
                        inputValidator: (value) => {
                            if (!value || !value.trim()) {
                                return 'Debes escribir el motivo de la desactivacion.';
                            }
            ...
            $('.form-reset-usuario').on('submit', function(event) {
                event.preventDefault();
