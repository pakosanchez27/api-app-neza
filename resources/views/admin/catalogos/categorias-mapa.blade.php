...
@section('content')
        ...
        <div class="bg-[linear-gradient(135deg,#2f1821,#61102a)] px-6 py-6 text-white">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <h2 class="mt-3 text-2xl font-semibold">Categorias mapa</h2>
                    <p class="mt-2 text-sm leading-7 text-white/78">Administra los registros de la tabla categorias_puntos.</p>
                </div>
                <a href="{{ route('admin.catalogos.categorias-mapa.create') }}"
    ...
    <div class="admin-shell admin-table p-5">
        ...
        <div class="overflow-x-auto">
            <table id="tabla-categorias-mapa" class="display stripe hover w-full text-sm">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Tipo</th>
                        <th>Puntos relacionados</th>
                ...
                <tbody>
                    @foreach ($categorias as $categoria)
                        <tr>
                            <td>{{ $categoria->id }}</td>
                            <td>{{ $categoria->tipo ?: 'Sin tipo' }}</td>
                            <td>{{ number_format($categoria->puntos_mapa_count) }}</td>
                            <td>{{ optional($categoria->created_at)->format('Y-m-d H:i') ?: 'Sin fecha' }}</td>
                            <td>
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.catalogos.categorias-mapa.edit', $categoria) }}"
                                    ...
                                    <form method="POST"
                                        action="{{ route('admin.catalogos.categorias-mapa.destroy', $categoria) }}"
                                        class="form-delete-catalogo inline-flex"
                                        data-message="Se eliminara la categoria {{ $categoria->tipo ?: 'seleccionada' }}.">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
...
@push('scripts')
    ...
    <script>
        $(function() {
            ...
            $('#tabla-categorias-mapa').DataTable({
                language: {
