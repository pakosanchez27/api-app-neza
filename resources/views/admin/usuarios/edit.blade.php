    ...
    <div class="mx-auto max-w-5xl space-y-5">
        <div class="overflow-hidden rounded-[26px] border border-[#eadde2] bg-white shadow-[0_18px_40px_rgba(97,18,50,0.07)]">
            <div class="bg-[linear-gradient(135deg,#2f1821,#61102a)] px-6 py-6 text-white">
                ...
                <h2 class="mt-3 text-2xl font-semibold">Editar usuario</h2>
                <p class="mt-2 text-sm leading-7 text-white/78">Actualiza los datos principales del usuario registrado.</p>
            </div>
            ...
            <div class="grid gap-6 px-6 py-6 lg:grid-cols-[220px_1fr]">
                <div class="rounded-[24px] border border-[#efe2d7] bg-[#fffaf6] p-5">
                    <div class="flex flex-col items-center text-center">
                        ...
                        @else
                            <div
                                class="flex h-28 w-28 items-center justify-center rounded-full bg-[#f3e6db] text-2xl font-semibold uppercase text-[#7a2144]">
                    ...
                    <form method="POST" action="{{ route('admin.usuarios.update', $user) }}" class="space-y-5">
                        ...
                        <div class="grid gap-5 md:grid-cols-2">
                            <div>
                                ...
                                <input type="text" id="nombre_p" name="nombre_p" value="{{ old('nombre_p', $user->nombre_p) }}"
                                    class="w-full rounded-2xl border {{ $errors->has('nombre_p') ? 'border-rose-400 bg-rose-50' : 'border-[#e8d9cb] bg-[#fffdfa]' }} px-4 py-3 text-sm text-[#201815] outline-none transition focus:border-[#63102a] focus:ring-2 focus:ring-[#63102a]/15">
                                @error('nombre_p')
                            ...
                            <div>
                                ...
                                <input type="text" id="app_p" name="app_p" value="{{ old('app_p', $user->app_p) }}"
                                    class="w-full rounded-2xl border {{ $errors->has('app_p') ? 'border-rose-400 bg-rose-50' : 'border-[#e8d9cb] bg-[#fffdfa]' }} px-4 py-3 text-sm text-[#201815] outline-none transition focus:border-[#63102a] focus:ring-2 focus:ring-[#63102a]/15">
                                @error('app_p')
                                    <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="apm_p" name="apm_p" value="{{ old('apm_p', $user->apm_p) }}"
                                    class="w-full rounded-2xl border {{ $errors->has('apm_p') ? 'border-rose-400 bg-rose-50' : 'border-[#e8d9cb] bg-[#fffdfa]' }} px-4 py-3 text-sm text-[#201815] outline-none transition focus:border-[#63102a] focus:ring-2 focus:ring-[#63102a]/15">
                                @error('apm_p')
                                    <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
                                @enderror
                            </div>
