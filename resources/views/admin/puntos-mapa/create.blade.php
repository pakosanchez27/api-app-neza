...
@section('title-section', 'Crear Punto Mapa')
@section('description', 'Registra un nuevo punto en el mapa con su categoria, ubicacion e imagen principal.')
...
@section('content')
    <div class="w-full rounded-[24px] bg-white p-6 shadow-[0_24px_60px_rgba(32,24,21,0.12)]">
        <div class="mb-6 flex items-start justify-between gap-4 border-b border-[#efe6dd] pb-4">
            <div>
                <h2 class="text-xl font-semibold text-[#201815]">Agregar Punto</h2>
                <p class="mt-1 text-sm text-[#7d6870]">Completa la informacion para publicar un nuevo punto en el mapa.</p>
            </div>
        ...
        <form class="space-y-6" enctype="multipart/form-data" method="POST" action="{{ route('admin.puntos-mapa.store') }}">
            @csrf
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="nombre_punto" name="nombre_punto" value="{{ old('nombre_punto') }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="descripcion" name="descripcion" rows="4"
                        class="w-full rounded-2xl border {{ $errors->has('descripcion') ? 'border-rose-400 bg-rose-50' : 'border-[#e8d9cb] bg-[#fffdfa]' }} px-4 py-3 text-sm text-[#201815] outline-none transition focus:border-[#63102a] focus:ring-2 focus:ring-[#63102a]/15">{{ old('descripcion') }}</textarea>
                    @error('descripcion')
                        <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div id="foto-preview-wrapper"
                        ...
                        <img id="foto-preview" src="" alt="Vista previa de la foto principal"
                            class="max-h-[420px] max-w-full object-contain">
                    </div>
                    <p id="foto-preview-empty" class="mt-3 text-[13px] leading-6 text-[#6f6166]">
                ...
                <div>
                    ...
                    <select id="categoria_id" name="categoria_id"
                        ...
                        @foreach ($categorias as $categoria)
                            <option value="{{ $categoria->id }}" @selected(old('categoria_id') == $categoria->id)>{{ $categoria->tipo }}</option>
                        @endforeach
                    </select>
                    @error('categoria_id')
                        <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" id="numero_exterior" name="numero_exterior" value="{{ old('numero_exterior') }}"
                ...
                <div>
                    ...
                    <input type="text" id="numero_interior" name="numero_interior" value="{{ old('numero_interior') }}"
                ...
                <div>
                    ...
                    <input type="number" step="0.00000001" id="latitud" name="latitud" value="{{ old('latitud') }}"
                ...
                <div>
                    ...
                    <input type="number" step="0.00000001" id="longitud" name="longitud" value="{{ old('longitud') }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="horarios" name="horarios" rows="3"
                        class="w-full rounded-2xl border {{ $errors->has('horarios') ? 'border-rose-400 bg-rose-50' : 'border-[#e8d9cb] bg-[#fffdfa]' }} px-4 py-3 text-sm text-[#201815] outline-none transition focus:border-[#63102a] focus:ring-2 focus:ring-[#63102a]/15">{{ old('horarios') }}</textarea>
                    @error('horarios')
                        <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fotoInput?.addEventListener('change', function(event) {
                ...
                if (!file) {
                    fotoPreviewWrapper.classList.add('hidden');
                    fotoPreviewWrapper.classList.remove('flex');
