@extends('layouts.app')
@section('title', $isEditing ? 'Editar categoria mapa' : 'Crear categoria mapa')
    ...
    <div class="mx-auto max-w-3xl">
        <div class="overflow-hidden rounded-[26px] border border-[#eadde2] bg-white shadow-[0_18px_40px_rgba(97,18,50,0.07)]">
            <div class="bg-[linear-gradient(135deg,#2f1821,#61102a)] px-6 py-6 text-white">
                ...
                <h2 class="mt-3 text-2xl font-semibold">{{ $isEditing ? 'Editar categoria mapa' : 'Crear categoria mapa' }}</h2>
            </div>

            <form method="POST"
                action="{{ $isEditing ? route('admin.catalogos.categorias-mapa.update', $categoria) : route('admin.catalogos.categorias-mapa.store') }}"
                class="space-y-5 p-6">
                @csrf
                @if ($isEditing)
                    @method('PUT')
                ...
                <div>
                    <label for="tipo" class="mb-1 block text-sm font-medium text-[#3e2d31]">Tipo</label>
                    <input type="text" id="tipo" name="tipo" value="{{ old('tipo', $categoria->tipo) }}"
                        class="w-full rounded-2xl border {{ $errors->has('tipo') ? 'border-rose-400 bg-rose-50' : 'border-[#e8d9cb] bg-[#fffdfa]' }} px-4 py-3 text-sm text-[#201815] outline-none transition focus:border-[#63102a] focus:ring-2 focus:ring-[#63102a]/15">
                    @error('tipo')
